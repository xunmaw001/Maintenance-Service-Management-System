    ...
    <fieldset class="search-container" style="text-align: center;margin: 50px 0;">
      <form class="layui-form" style="margin: 10px 0;">
        ...
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('qichexinxi','新增')" @click="jump('../qichexinxi/add.jsp')" type="button" class="layui-btn btn-theme">
      ...
      <div class="box">
        <ul class="clearfix">
          <li v-for="(item,index) in dataList" v-bind:key="index">
            <a :href="'../qichexinxi/detail.jsp?id='+item.id">
              <figure>
                <img :src="item.tupian?item.tupian.split(',')[0]:''" width="410" height="272" />
              </figure>
              <h2>{{item.qichexinghao}}</h2>
              <div class="div1">
                <b></b>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#qichexinghao').val()) {
          param['qichexinghao'] = jquery('#qichexinghao').val() ? '%' + jquery('#qichexinghao').val() + '%' : '';
        }  
        if (jquery('#chexing').val()) {
          param['chexing'] = jquery('#chexing').val() ? '%' + jquery('#chexing').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('qichexinxi/list', 'get', param, function(res) {
